...
import { usePageScripts } from '../usePageScripts';
														        		...
														        		<input type="email" name="email" value="" size="40" class="form-control" aria-required="true">
																			...
																			<div class="row">
																				<div class="col-sm-12">
																					...
																					<span class="form-control-wrap"><input type="text" name="subject" class="form-control" aria-required="true" required></span>
																				</div>
																			</div>
																			<div class="row">
																<div class="col-sm-12">
																	...
																	<span class="form-control-wrap">
																		<textarea name="message" cols="40" rows="10" class="form-control" aria-required="true"></textarea>
																			...
																			<p class="contact-feedback" role="status" style="margin-top:15px"></p>
														</div>
													</form>
												</div>
											</div>
...
export default function Page_page_contact() {
  const ref = useRef<HTMLDivElement>(null);
  usePageScripts(ref, 'Contact | Mojuri');
  useEffect(() => {
    ...
    const submit = async (event: SubmitEvent) => {
      ...
      try {
        const result = await api<{ message: string }>('/contacts', { method: 'POST', body: JSON.stringify({ name: values.get('name'), email: values.get('email'), subject: values.get('subject'), message: values.get('message') }) });
        form.reset();
        if (feedback) { feedback.textContent = result.message; feedback.style.color = '#2f7d42'; }
      } catch (error) {
        if (feedback) { feedback.textContent = error instanceof Error ? error.message : 'Unable to send your message.'; feedback.style.color = '#b3261e'; }
      } finally { if (button) { button.disabled = false; button.value = 'Submit'; } }
    };
